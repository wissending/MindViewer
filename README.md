...
- [MindViewer](#mindviewer)
  ...
  - [TGAM通信协议](#tgam通信协议)
    - [前言](#前言)
    - [蓝牙接口](#蓝牙接口)
  - [ThinkGear数值](#thinkgear数值)
    - [POOR_SIGNAL Quality](#poor_signal-quality)
    - [eSense™计量器](#esense计量器)
  ...
  - [ThinkGear数据包](#thinkgear数据包)
    ...
    - [源数据格式](#源数据格式)
      - [编码定义表](#编码定义表)
  - [示例包](#示例包)
    - [一步一步分析数据包指南](#一步一步分析数据包指南)
    - [数据包有效负载中数据路的分步解析指南](#数据包有效负载中数据路的分步解析指南)
    - [数据包解析样本C代码](#数据包解析样本c代码)
